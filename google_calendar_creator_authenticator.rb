...
# GoogleCalendarCreatorAuthenticator handles the OAuth 2.0 authentication flow
# for Google Calendar API with write permission
class GoogleCalendarCreatorAuthenticator
  ...
  # Google Calendar API用のOAuth 2.0認証を実行する（書き込み権限付き）
  #
  ...
  def perform_authentication(authorizer, user_id)
    puts "=== Google Calendar OAuth 2.0 Setup (Write Permission) ===\n",
    ...
    puts "\n",
         ...
         "You can now run 'ruby google_calendar_creator.rb' to create calendar events."
  end
...
if __FILE__ == $PROGRAM_NAME
  begin
    authenticator = GoogleCalendarCreatorAuthenticator.new
